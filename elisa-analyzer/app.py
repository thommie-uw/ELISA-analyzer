...
with st.sidebar:
    ...
    rec_low, rec_high = st.slider(
        "Standard Recovery Limits (%)", 50, 150, (80, 120), 5,
        help="Each standard's mean OD is read back down the fitted curve and compared "
             "with the concentration you pipetted: recovery % = back-calculated ÷ nominal "
             "× 100. Levels landing outside this window are flagged 'Out of Limits' in the "
             "Standards table. It is a QC flag only — widening it changes no reported "
             "result. 80–120% is the usual window.",
    )
...
if result is not None:
    ...
    with tab_curve:
        ...
        st.caption(
            f"**Recovery %** = each level's mean OD read back down the curve, divided by "
            f"the concentration you pipetted. 100% is perfect; anything outside "
            f"{rec_low}–{rec_high}% is flagged. A level that fails with a *low* %CV is "
            "usually a mis-made standard — drop the level. Failing with a *high* %CV "
            "points at one bad well instead."
        )
        st.dataframe(
            result.standards,
            hide_index=True,
